...
Targets MILO_URL (default http://127.0.0.1:8767, so start a second server on that port rather than
probing the one you talk to). Uses the synthetic milo-hello.wav fixture written by setup_voices.py;
never opens a microphone. Writes evidence/local/latency-<date>-<label>.json (gitignored).
...
from milo import config as _config

BASE = os.environ.get('MILO_URL', 'http://127.0.0.1:8767')
FIXTURE = Path(os.environ.get('MILO_FIXTURE', str(Path(_config.load()['models_dir']) / 'milo-hello.wav')))
...
def gpu_state():
    try:
        with urllib.request.urlopen(_config.load()['ollama_url'].rstrip('/') + '/api/ps', timeout=5) as r:
            ps = json.load(r)
...
def main():
    ...
    path = Path(__file__).resolve().parents[1] / 'evidence' / 'local' / f'latency-{stamp:%Y%m%d}-{args.label}.json'
